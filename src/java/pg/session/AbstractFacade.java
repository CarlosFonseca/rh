...
//------------------------------------------------------------------------------------------------     
    ...
    public List<Tuple> findCriteria(int[] range, CompoundSelection<Tuple> compoundSelection, Predicate[] condicao, Order[] Ordencao, Expression[] compoundGroup) {
     ...
     public List<Tuple> findCriteria(int[] range, CompoundSelection<Tuple> compoundSelection, Predicate condicao, Order[] Ordencao, Expression[] compoundGroup, Predicate condicaoHaving) {
        javax.persistence.criteria.CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
        javax.persistence.criteria.CriteriaQuery cq = cb.createQuery();
        cq.select(compoundSelection);
        if(condicao!= null){
   ...
   //------------------------------------------------------------------------------------------------     
    ...
    public List<T> findNamedQuery (String sqlNamedQuery,String[] parametros, Object[] valores  ){
        javax.persistence.Query q = getEntityManager().createNamedQuery(sqlNamedQuery);
         for (int i = 0; i < parametros.length; i++) {
             q.setParameter(parametros[i], valores[i] );
           }
         return q.getResultList();
 }       
    
    public Object findSingleResult (String sqlNamedQuery,String[] parametros, Object[] valores  ){
        javax.persistence.Query q = getEntityManager().createNamedQuery(sqlNamedQuery);    
        for (int i = 0; i < parametros.length; i++) {
             q.setParameter(parametros[i], valores[i] );
          }
